local branch = getgenv().lonelyhub_dev_mode and "dev" or "main"

local HttpService = game:GetService("HttpService")
local baseURL = "https://raw.githubusercontent.com/LongHip2012/Lonely_Hub_Doors/" .. branch
...
if getgenv().lonelyhub_disable_addons then return end
task.spawn(function()
    ...
    if not supportsFileSystem then
        warn("[lonelyhub] Your executor doesn't support the FileSystem API. Addons will not work.")
        return
    end

    if not isfolder("lonelyhub/addons") then
        print("[lonelyhub] Addons folder doesn't exist. Creating...")
        makefolder("lonelyhub/addons")
        return
    end
    
    repeat task.wait() until getgenv().lonelyhub_loaded == true
    print("[lonelyhub] Loading addons...")
    ...
    local function AddAddonElement(LinoriaElement, AddonName, Element)
        if not LinoriaElement then
            warn("[lonelyhub] Element '" .. tostring(Element.Name) .. " (" .. tostring(Element.Type) .. ")' didn't load: Invalid Linoria element.")
            return
        end

        if typeof(Element) ~= "table" then
            warn("[lonelyhub] Element '" .. tostring(Element.Name) .. " (" .. tostring(Element.Type) .. ")' didn't load: Invalid data.")
            return
        end 

        if typeof(Element.Type) ~= "string" then 
            warn("[lonelyhub] Element '" .. tostring(Element.Name) .. " (" .. tostring(Element.Type) .. ")' didn't load: Invalid name.")
            return 
        end

        if typeof(AddonName) ~= "string" then 
            warn("[lonelyhub] Element '" .. tostring(Element.Name) .. " (" .. tostring(Element.Type) .. ")' didn't load: Invalid addon name.")
            return 
        end
        ...
        warn("[lonelyhub] Element '" .. tostring(Element.Name) .. " (" .. tostring(Element.Type) .. ")' didn't load: Invalid element type.")
    end

    local gameAddonPath = getGameAddonPath(scriptPath)
    print("[lonelyhub] Game addon path: " .. gameAddonPath)
    ...
    local function createAddonTab(hasAddons: boolean)
        ...
        local addonsText = "This tab is for UN-OFFICIAL addons made for lonelyhub. We are not responsible for what addons you will use. You are putting yourself AT RISK since you are executing third-party scripts."
    ...
    for _, file in pairs(listfiles("lonelyhub/addons")) do
        print("[lonelyhub] Loading addon '" .. string.gsub(file, "lonelyhub/addons/", "") .. "'...")
        ...
        local success, errorMessage = pcall(function()
            ...
            if typeof(addon.Name) ~= "string" or typeof(addon.Elements) ~= "table" then
                warn("Addon '" .. string.gsub(file, "lonelyhub/addons/", "") .. "' didn't load: Invalid Name/Elements.")
                return 
            end

            if typeof(addon.Game) == "string" then
                if addon.Game ~= gameAddonPath and addon.Game ~= "*" then
                    warn("Addon '" .. string.gsub(file, "lonelyhub/addons/", "") .. "' didn't load: Wrong game.")
                    return
                end
            elseif typeof(addon.Game) == "table" then
                ...
                warn("Addon '" .. string.gsub(file, "lonelyhub/addons/", "") .. "' didn't load: Invalid GameId.")
                return
            end
        ...
        if not success then
            warn("[lonelyhub] Failed to load addon '" .. string.gsub(file, "lonelyhub/addons/", "") .. "':", errorMessage)
        else
