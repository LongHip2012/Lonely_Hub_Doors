...
if not ExecutorSupport then print("[Lonely Hub | DOORS] Loading stopped, please use the official loadstring for Lonely Hub. (ERROR: ExecutorSupport == nil)") return end
...
function Script.Functions.File.BuildPresetStructure()
    if not isfolder("lonelyhub/doors/presets") then
        makefolder("lonelyhub/doors/presets")
    end
end
...
function Script.Functions.File.CreatePreset(name: string, data: table)
    ...
    writefile("lonelyhub/doors/presets/" .. name .. ".json", HttpService:JSONEncode(presetData))
end
...
function Script.Functions.File.LoadPresets()
    ...
    for _, file in pairs(listfiles("lonelyhub/doors/presets")) do
...
function Script.Functions.LoadPreset(name: string)
    ...
    local success, ret = pcall(function()
        local data = readfile("lonelyhub/doors/presets/" .. name .. ".json")
        return HttpService:JSONDecode(data)
    end)
...
if supportsFileSystem then
    local PresetGroupbox = Tabs.Main:AddLeftGroupbox("Presets") do
        ...
        PresetGroupbox:AddButton({
            Text = "Create Preset",
            Func = function()
                if isfile("lonelyhub/doors/presets/" .. Options.Elevator_PresetName.Value .. ".json") then
        ...
        PresetGroupbox:AddButton('Delete Preset', function()
            if not isfile("lonelyhub/doors/presets/" .. Options.Elevator_PresetList.Value .. ".json") then
                Script.Functions.Alert("Preset does not exist!")
                return
            end
    
            local success, err = pcall(function()
                delfile("lonelyhub/doors/presets/" .. Options.Elevator_PresetList.Value .. ".json")
            end)
...
SaveManager:SetFolder("lonelyhub/doors")
